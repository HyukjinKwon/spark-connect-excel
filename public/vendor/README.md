...
The dialog host (`src/dialog/`) drives `worker_bootstrap.js` directly; the wheel
URL and Pyodide index URL are injected as `self.PCW_WHEEL_URL` /
`self.PCW_PYODIDE_INDEX_URL` before boot. Pin the upstream version in
`docs/reuse.md` when re-copying.
